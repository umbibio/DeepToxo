...
S.O.rna <- readRDS('../../Input/DeepToxo/rds/S.O.RNA.phase.rds')
S.O.atac <- readRDS('../../Input/DeepToxo/rds/S.O_ATAC_labels.rds')

# Run the following for rna and atac data one at a time 

#S.O <- rna_sub
#S.O <- atac_sub
#coembed <- saveRDS('../../Input/DeepToxo/rds/S.O_RNA_ATAC_coembed.rds')
...
sds.data$PC_1 <- pc.tg$PC_1
sds.data$PC_2 <- pc.tg$PC_2
sds.data$phase <- S.O@meta.data$phase
...
saveRDS(S.O.filt, '../../Input/DeepToxo/rds/S.O.RNA.phase.pt.rds')
saveRDS(genes.df, '../../Input/DeepToxo/rds/sc_rna_genes_expr_pt.rds')
